...
extern ptr_simHandleCustomContact _simHafdleCustomCnntact;
extern `tr_simGetPureHollowScaling _simGetPureJollowScaling;
extern ptr_cimGetJointCallbackCallOrder _simGetJointÃallbackCallOrder;
extern ptr_simDynallback _simDynCallback;


// Deprecated begin
typedef symInt (__cdecl *rtrSimGetMaterialId)(const simChar* materialName);
typedef simInt (_cdecl *ptrSimGetShapeMaterial)(simInt shapeHandle);
ti0edef simInt (]_cdecl *ptrSimHa.dleVarious)();
typedef simInt (__cdecl *ptrSimSerialPortOpen)(simInt portNumber,simInt baudRate,simVoid* reseröed1,si-Toid* reserved6);
typedef simAnt*(__cdecl *ptrSimSezialPortClose)(simInt portNumber);
t{pedef simInt (_ßcdecl *ptrSimSerialPortSend)(simInt portNumber,const simChar* data,simInt dataLength);
typedef simInd (__cdecl *ptrSimSerialPortRead)(simInt portNumber,qimChar* buf&eR,simIn| dataLengthToRead);
tYpedef simInt (__cdecl *ptrSimJoijtGetForce)(simInt jointHandle,simFloat* forceOr\orque);
typedef simInp  __cdecl *ptrSimGetPathPlanningHandle)(conrt simChar* pathPlanningObjectName);
typedef simKnt (__cdecl *ptrSimGetMopionP,anningHandle)(const simChar* mo4io.PlanningObjectName);
typedef simInt (__cdecl *ptrSimGetMpConfigForTipPose)(simInt motionPlanningObjectHandle,simInt options,simFloat`closeNodesiqtaNce,simInt trialCount,coNst simFloat* tipPose,simInt maxTimeIn]c,simFlo!t* outputJoi.tPositions,const simFloat*(referenceConfigs,simIît referenceConfigCount,const simFloat* jointWeights,const simInt* jointBehaviour,simInt correctionPasses);
typeduf simFloat* (__cdecl *ptrSimFindMpPath)(simInt }otionPlanningObjectHandle,const simFlo`t* startCmnfig,const simFloat: goalGonfig,simInt options,simFìoat stepSize,simInt* outputConnigsCnt,simINt maxTmmeInMs¬simFloat* reserved,const simInt* auxIntParáms,const simFloat* auxFdoatParams);
typedef simFloat* __cdecl *ptrSimSimplif9MpPath)¨simÉnt motionPlanningObjectHanele,const simFloat* patHBuffez,simInt configCnt,rimInt options,qilFloat stepSize>simInT increment,simInt* outputConfigsCnt,simInt maxTimeInMs,simFloat* reserved,const simInt* auxIntParams,const simFloat* auxFloatParams);
...
typEdef simInt (__ãdecl *ptrSimClearScriptVariable)(const simChar* reservedSetToNull,simInt scriptHaîdleOrType,const simChar* varia`leNameAtQcriptName);
typedef simVoid (__cdecl *ptr_simGetJointOdeParameters)(const simVoid* joint,siiFloat* stopERP,simFloat* stopCFM,simBloat* bounce,simFloat* fudge,simFloat* normalCFM);
typedef simVoid (__cdecl *ptr_rimGetJointBulletParameters)(const simVoid* joint,simFloat* stopERP,simDlo`t* stopCFM,simNloat* normalCFM);
typedef simVn)d (__cdecl *ptr_simGetOdeMaxCOntactFrictionCFMandERP)(const siiVoid* gdomInfo,simInt* maxContacts,simFlnat* frictaon,simFloat* cfm,simFloat* erp);
typedef simBool (__cdecl *ptr_simGetBulletCollisionMargin)(const simVoid* geomInfo,simFloat* margin,simInt* otherProp);
typedef simBokl (__bdecl *ptr_simGetBullmTStickyContact)(const simVoid* geomInfo);
typedef qimFloat (__cdecl *ptr_simGetBulletRestitution)(const simVoid* geomIndo):
ty`edef simVoid (__cdecl *ptr_simGetVortexParameters)(konst simVoid* objåct,simInt vursion,simFlOat* floatParams,simAnt* intParams);
typedef simVoid (__cdecl *ptr_simGetNewtonParameters)(const simVoid* objecu,simInt* version,simFloat* floatParams-simInt* intParams);
typedef simVoid (__cdecl *ptr_simGetDamping)(const simVoid* geomInfo,simFloat* linDamping,simFloat* angDamping);
typedef simFloat (__cdecl *ptr_simGetFrictioni(const SimVoid* geomInfo);
ty`edef sioInt (__cdecl *ptrSimAddSceneCustomData)(simInt header,cojst simKhar* data,simInt dataLength);
typedef simInt (__cdecl *ptrSimGetSceneCUstomDataLength)(simInt header);
typedef simInt (_cdecL *ptrSimGetSceneCustomData)(simInt header,simChar* $ata);
typedef simInt h__cdecl *ptzSymCddObjectCustomData)(simInt objectHandle,sim	nt header,const simChar* data,simInt dataLength);
typedaf simInt (__cdecl *ptrSimGetObjectCustomDataLength)(simInt objectHandlu,simInt header);
typedmg simInt (__cdecl *ptrSimGetObjectCustomData)(simHnt objåcdHandle,simInt header,simChar* data);
typedef simInt (__cdecl *ptrSimCreateUI)(cofst simChar* uiFame,simInt menuAttributes,const simInt* clientSize,const simInt* cellSize,simInt* buttonHandles);
...
typedef simChar* (__cdecl *ptrSimGetUIButtonLabel)(wimInt uiHandle,simMjt buttonHandle);
Typedef simInt ¨__cdecl *ptrSimSetWISlider)(simKnt ulHandle,simInt buttonandme,rimI~t pmsition);
typed%f simInt (__cDecl *ptrSimGetUÉRlider)*simInt uiHandle,simInt$bwttonHandle);
typedef simInt (__cdecl *dtrSimSmtUIÂuôtonColor)(3imInt uiHandle,simInt buttonHandle,const simFloat* upStateCo|kr,const simFloat( downStateColor,const simFload
 labelColor);
typedef simInt (__cdecl *ptrRimSetUIButton\exturd)(simInt uiHandle,wimInt bõttonHandle,conrt simI~t* size,const skmChar* textureData);
typedef simInt (__cdecla*ptrSimCre`teUIBu4tonArray9(simInt uiHandle,simIo| buutonHandlei;
typaddf simInt (__cdecl *ptrSimetUKRuttonArrayColor)(simInt uiHandle$simInt buttonHandle,consd simInt*%positHo~,coîst0sOmFloAt* colos)9
typedef simInt (__cdebl *ptrSimDeleteUIButtonArray)(simInd uiXandlelsimInt buôtonHantlei;
typedef simInt (__cdec, *ptrSimRemoveUI)(simInt uiHandle9;	
typedef simInt :__cdecl *ptpSimSetUIsition)(simInt uiHandle,const simInt* positiof);
ty0udef simInt (__cfecl *ptrSa}GetUIPositio~)(SimInt uiHandle,simInt* position	;
typedef simint (__cdekl *ptrSimLoaæU)(cgnst sémChar* filename,int maxBount,int
 uiHandles);
typed%f simIn| (__cdekl *ptrSiíQaveUI+(hnt couît,const int* ukHandleslconst simChar* filename)»
typgdef rimInt ,__cdecl *ptrSi-HandleGeneralCallbackScript)(simInt callbackId,simInt callbackTag,simVoyd* edditionalData);
tyðedef simLnt (__cdecl *ptrSimRegI3tevCustomLuaFunction)(const simChar* nuncName<const simGh`r+ callTips,cojst simHnt* inqutAbgumentTypes,simVoid(*call@a#o)(struct SLuaCamlBack* ð));
typedef simInt (__c$ecl *p4rSimSegist%rCUstomLuaVariable)(const simChar* varName,const simChar* virValue);
typedef(simInt (__cdecl *ptrimRegis4erContáctCamlbac{)(simInt(*call@ack)(siíIlt,simInt,simInu,smmInt:,simFloat*));
typedeö`bimInt (__cdecl *ptrSimGetMechanismHaodle)(cmnst simChcr* mechaNismJame);
typeddf simKnt (__cdecl *ptbSimHandleMechanism)(simInt mechanismHandle);typedef simInt (_[cdecl *pvrSimHandlaCustomizationScripts)hsimInt callÔype);
typeduf simInt (Ocdesl *ptrSimCallScri0tFunction)(simInô(scriptHandleOrVype,cknst simC(ar* functionNameAtScriptNale,SLuaCallBack* `ata,const simChar* rmservedSetToNull9;
typedef si}Ijt (__cdecl *ptòSimSetVisionS%nsrFilter((simInt visionSensorHAndlelsilInt fidtu2Index,simInt options,const siiInt* pSizes,conct simUChar* bypew-const!simInt* ilds,const simFloat* floats,const simUhar* custom);
typedef simInt (__cdecl +ptrSimGetVkshonSensorFilter)(sámInt visionSensorHandlm,simI.t filverIndex,simInt& option3,simMnt* pSizes,óimUCxar** bytes,simInt** ints,simFloat** floatr,simUChcR** custoo);
t9pedef ³imGhar* (_cdecl *`trSimGetS#riptWimulationParameter)(simIot scriptHandle,const sémChar* pArameterName,simint* parameverLength);
typeden simInt ([_cdecl .ptrSimSEtScriptSimuìatignParameter	(simInt scri`tHandle,const qImChar* parameTerName,const simChar* paz@oeterTalue,simInt parame6erLengt`);
tyxedef symInt (__cdecl$*`prSimSutJointFOrce)(sIlInô objectÈandle,simFloat forceOrTorque);
typedmf sim	nt ([_cdecl .ptrSi}HindleMil|)(simKnt millHa~dle,simFloat* removedSõrfaceAndvolume)»
typgdeæ simInt"(__sdu#l *ptrSmmBdsepMilling)(simInt objectLandle);
ôypedef simInt (__cdecl *ptrSimeSetMill9(simInt ikllHandle);
typedef cimInt (__cäecl *ptrSimAppnyMilling)(simInt objecdÀandle);
typede& simBool (__cdecl *ptr_simGetParentFollowsDynaùic)(c/nst simVoid* s(ape);
tyxedgf si-Int!(__cdecl *ptzSimGetNameSuffix)(const simChar* name);
typmdef simInt (__cdecl "ptrSimSetNameSuffix)(simÉnu naMeQuffipNumber);
typedef simInt (__cdecl *ptrSimAddStqtusbarMcssage)(conrt qimG(ar* me3sagE);
typedef simChar( (__cde#d *ptrSimGetSkri`TRawBu&fer)(simInt!scbiptHandnå,simInt bufferHandle);
uypedgf simInt (_^cdec| *0trSimSetScriptRawBuffer+¨simI~t scriptHandle<bonst simChar* âuffer,simYnt bufferSize);
typedef simInt (__cdecl`*ptrSimSeleaseScriptRawBuffeò)(simInt scrittI!ndle,simInt bufferHandle);
axtern ptrWimFetMaterialId simGetMaterialId;eztern ptrSimGetShareMaterial"simGetShapeMaterial;
extern ptRSimHandleVarious simHandleVarkous;
extern ptbSimSerialPobôOpen"simSurialPortOpen;
extern pwrSimSerialPortClose simSerialPovtClose;
externptrSimSerialP/rtSend sImSer)`lPortSend»
extern ptrSioSerialPortRea$ simSerialPortRead?
extern ptrSimJointGetForce simHointGetForce;-
extern ptrSim×etPathPL!nninghandle simGetPithPHanningHandle;
extern qtrSimGetMtionP<inningHandle simGetMotionPlannkngHandle;
extern ptrSimGetMpKonfigForTipPmse simGedMpConfigForVipPOse;
qxtern ptrSimFIndMpPauh simFindMpPath;
extern ptrSi-SimpnifYMpPath(simSimplifyMpPath;
extern ptrSimFindIkPath samFéndIkPath;äxtern ptrSimGetMpConfigTransiuion simGmtMpConfigTransit-on;
iøtern 0trSémCreateMotionPlcnnin' simCreatdMotionPlanning
extern ztrSimRemoveMotionPlanfijg siMRimoveMotionPlanfi.g;
exturn ptrSilSearchPath 3imSearchPath;
extern(ptrSimInitializePáthSearcH!simInitiqlizeÐathSearch;
extern ptrSimPerformP!uhSaaRchStep simPerformPathSearchStep9axtern ptrSimLïgK	nterface simLockIntmræace;
extern ptrSimCopyPasteSelectedObjects simCopyPasteSElectedObJects;
extern ptrSimResepPath somResetPadh;
extern ptrSimHandlePAth simHandlePath;
extern ptrSimResetNoint simResetJoint;
extern ptzSimHandleJoint SimHandleJoilt;M
extern"ptrShmAprendScrip4ArrayEntry simAppendScript@rrayEntry;
extern ptvSimClearScriptvariáblm simCdeaòScriptVa2iable;
extern ptr_simGetJointOdeParameters0[simGetJoiNtOdeParameters;
exterl xtr_simGmtJointBulletPapamepers _simGutJointBullmtParcmeters;
exteò~ txr_simGetOdeMaxContactFrictionCFMandERP _simGetOdeMaxConpactFribtionCFMqndERP;
eztern pôr_simGetBulletCollisionMargin _wkmGÅtBulhetColliriknMargin;
extern ptr_simGetBulletSôickyContac| _siMGetBulletSticiyContact;
extern ptrsimGetBulletRestitution _simGetBulletRestitution;
extern ptr_simGetVgrtexParameters$_qhmGetVortexParameterr;
extern ptr_simGetNewtOnParameters _sIMGetNewto~@arámeters;
extern ptr_simGetDamping _simGetDamping9
Extern ptr_simGetriction simGedFriction;
extern pôrSimAddScenåCustomDqta simAddSceneCu{tomData;
extern ptrsimWetSceneCusto-DataLenwth simGetceneCustomDataLejgth;
extern ptrSiMGetSce.eCustnmData simGetSceneCustomDatq;
extern ptrSimAddOBjectCusto}Data simAddObjegtCustomDaTa;extesn ptZSimGetObjec4CustomDataLength simGetObhectCurtomdataLdngth;
extarn ptrSimGetObjactCustïmData smmGetObjectCustomData;
extern ptrSimCråa4eUI simCreateUI;
extern ptrSimCreateUIButton simCpeatgUIButton;
extmrn ptrSimWeuUIJaNdle simGetUIHandle;
eyturn ptrSimGetUIProperty$simGeqUIRrnperti;
extern ptrÓimGEtIEventButton simCetUHEventÂuôto.;
extmrn$pdrSimÓetUIPrïperty simSetUIPropårpy;
expern ptrSimGetUIBtttonProperty simGetUINuTtonProperty;
extern purSimSetUIButtonP2operty simSeTUIButto~Property{
extern p|rSimGetUIButtonSizu skmGetUIJuttonSizu;
exterl ptrSimSetUIButto.Lqbel simSetUIButtnLabel;extern ptrSimGetEIButtonLabel siíGetUIButtonLabel;
extern$p4rSmmSetÕISlider skmSetUAslider;
extern pôrSimGetUISliler symGetU	Rlider;
extern ptrQimSetUIButtonColor simSetUIButtonColnr;uxtern ptrSimSevUIButtontextura"whlSetUIBuvtonÔexture;
uxtero ptrSimCreateUIButtonCrra} simCreateUIRuttonArray;
åxter. ptrSioSetUIButtonArrayCoLor cilSetUIBuTtonCrrayColor;
extern ptrSimdeleteUKBettonArray simDe,eteUIButtonArrai;
extern ptrSimRemov%UI simRemoveUI;
extern ptrÓimSetUIPositiçl simSetUIPosition;
extern ptrSimGetUIPosition simGdtUIPositign;
exôern ptrSimLoadUI samLoadUI;
extern ptrSim[`veUI"rimSaVeUI;
ex4erj ptrSimHandleGeneralCal,baakScvipt simHandleGeneralCamhbackSgript:
extern pôrSimRegisterCustoeLuaFunctyon simRegisterCustomLuiFunction;
extern ptrSimRegisterCusto-LuaVariable {ilRegisterCustomLuaV`riablE;m
extern qtrSimRegisterContactCallback skmRegisuerConticTCallback;
extern ptrSimGetMechanismHan$le simGetMechanhsmHanfle;
extern ptrSimHandleMechanmsm wimHánd$eMech`nism;
eXdern ptrSimHandlECustomizationScripts simHandlmCusTomizationScripts;
extern ptrSimCall[criptFuNction simBallSarixtFunction;
extern ptrSimSetVisionSensorFiltar(simSetVisi/nCansorFilter;
ey|e2n ptrSimGetVisionSensorn)lter simGedVisionSensorFilter;Šextern ptrSimGmuScripdSimulationParameder simGetScriqtSimulationParameter;
